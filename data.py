"""
1. data.py
Features are normalized [0,1] and combined into a 1D tensor
"""
...
# For loading dataset: expert actions -> labels for training
ACTION_TO_INDEX = {action: index for index, action in enumerate(ACTIONS)}

# Pacmanagent.py while run: predictions -> game actions
INDEX_TO_ACTION = {index: action for action, index in ACTION_TO_INDEX.items()}


def state_to_tensor(state: object) -> torch.Tensor:
    """
    Transform 1 GameState to 32 normalized features:
    ...
    if food_positions:
        ...
        closest_food_index = distances_to_all_foods.index(
            min(distances_to_all_foods)
        )
        closest_food_x, closest_food_y = (
            food_positions[closest_food_index]
        )
        closest_food_dist = (
            float(distances_to_all_foods[closest_food_index])
        )  # 5
        dist_to_food_x = float(closest_food_x - pac_pos_x)  # 3
        dist_to_food_y = float(closest_food_y - pac_pos_y)  # 4

        # Flag in 4 directions
    ...
    # Dist from center
    dist_to_center_x = float(pac_pos_x - center_x)  # 1 if + = right of center
    dist_to_center_y = float(pac_pos_y - center_y)  # 2 if + = upper side
    ...
    # Lane = 2 opposite directions
    is_lane = 0.0
    if num_legal_moves == 2:
        has_north_south = (Directions.NORTH in legal_actions
                           and Directions.SOUTH in legal_actions)
        has_east_west = (Directions.EAST in legal_actions
                         and Directions.WEST in legal_actions)
        is_lane = 1.0 if (has_north_south or has_east_west) else 0.0  # 8
    # Open space = 3-4 legal moves
    is_open_space = 1.0 if num_legal_moves >= 3 else 0.0  # 9

    # Legal actions vector flag(5)
    legal_flags = []
    ...
    # To normalize : dist corner to corner
    ...
    features = [
        ...
        # Ghost (7)
        dist_to_ghost_x / float(maze_W),
        dist_to_ghost_y / float(maze_H),
        ghost_mantt_dist / float(max_manhattan_dist),
        ghost_in_north,
        ghost_in_south,
        ghost_in_east,
        ghost_in_west,
...
class PacmanDataset(Dataset):
    """
    PyTorch Dataset for Pacman expert demonstrations

    Load (state, action) & converts them to tensors for training
    """
